...
public class APITest {
	...
	/**
	 * Test that merkle links in values of a cbor map are followed during
	 * recursive pins
	 */
	@Test
	...
	/**
	 * Test that merkle links as a root object are followed during recursive
	 * pins
	 */
	@Test
	...
	public void swarmTest() throws IOException {
		...
		if (addrs.size() > 0) {
			boolean contacted = addrs.keySet().stream().anyMatch(target -> {
				try {
