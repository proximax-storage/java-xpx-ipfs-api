...
public class IPFS {
	...
	public List<MerkleNode> add(List<NamedStreamable> files, boolean wrap, boolean hashOnly) throws IOException {
		Multipart m = new Multipart("http://" + host + ":" + port + version + "add?w=" + wrap + "&n=" + hashOnly,
				"UTF-8");
		for (NamedStreamable file : files) {
			if (file.isDirectory()) {
				m.addSubtree(Paths.get(""), file);
	...
	public class Pubsub {
		...
		public Supplier<CompletableFuture<Map<String, Object>>> sub(String topic, ForkJoinPool threadSupplier)
				throws Exception {
			Supplier<CompletableFuture<Object>> sup = retrieveAndParseStream("pubsub/sub?arg=" + topic, threadSupplier);
			return () -> sup.get().thenApply(obj -> (Map) obj);
		}
		 ...
		 * @throws IOException
		 */
		public void sub(String topic, Consumer<Map<String, Object>> results, Consumer<Throwable> error)
				throws IOException {
	...
	public class Block {
		...
		public MerkleNode put(byte[] data, Optional<String> format) {
			...
			Multipart m = new Multipart(
					"http://" + host + ":" + port + version + "block/put?stream-channels=true" + fmt, "UTF-8");
	...
	public class IPFSObject {
		public List<MerkleNode> put(List<byte[]> data) throws IOException {
			Multipart m = new Multipart("http://" + host + ":" + port + version + "object/put?stream-channels=true",
					"UTF-8");
			for (byte[] f : data)
		...
		public List<MerkleNode> put(String encoding, List<byte[]> data) throws IOException {
			...
			Multipart m = new Multipart(
					"http://" + host + ":" + port + version + "object/put?stream-channels=true&encoding=" + encoding,
					"UTF-8");
			for (byte[] f : data)
				m.addFilePart("file", Paths.get(""), new NamedStreamable.ByteArrayWrapper(f));
			String res = m.finish();
		...
		public MerkleNode patch(Multihash base, String command, Optional<byte[]> data, Optional<String> name,
			...
			case "append-data":
				...
				Multipart m = new Multipart("http://" + host + ":" + port + version + "object/patch/" + command
						+ "?arg=" + base.toBase58() + "&stream-channels=true", "UTF-8");
	...
	public class Dag {
		...
		public MerkleNode put(String inputFormat, byte[] object, String outputFormat) throws IOException {
			...
			Multipart m = new Multipart(
					prefix + "block/put/?stream-channels=true&input-enc=" + inputFormat + "&f=" + outputFormat,
					"UTF-8");
	...
	public class Config {
		...
		public void replace(NamedStreamable file) throws IOException {
			Multipart m = new Multipart("http://" + host + ":" + port + version + "config/replace?stream-channels=true",
					"UTF-8");
			m.addFilePart("file", Paths.get(""), file);
			String res = m.finish();
		}
	...
	private Supplier<CompletableFuture<Object>> retrieveAndParseStream(String path, ForkJoinPool executor)
			throws IOException {
		BlockingQueue<CompletableFuture<byte[]>> futures = new LinkedBlockingQueue<>();
		CompletableFuture<byte[]> first = new CompletableFuture<>();
		futures.add(first);
		executor.submit(() -> getObjectStream(path, res -> {
	 ...
	 * @throws IOException
	 */
	private void retrieveAndParseStream(String path, Consumer<Object> results, Consumer<Throwable> err)
			throws IOException {
